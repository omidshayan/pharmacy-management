...
$income = max(0, (float)$fund['income']);
$totalFund = max(0, (float)$fund['total']);
$total = $income + $totalFund;
?>

<!-- Start content -->
<div class="content">
    <div class="content-title"> مدیریت صندوق‌ها
        <span class="help fs14 text-underline cursor-p color-orange" id="openModalBtn">(راهنما)</span>
    </div>
    <?php
    $help_title = _help_title;
    $help_content = '
        شما می توانید موجودی دخل را به صندوق اصلی انتقال دهید!
    ';
    ...
    <div class="mini-container mt15">
        <div class="details">
            <div class="detail-item d-flex align-center">
                <div class="w100 m10 center">موجودی دخل</div>
                <div class="w100 m10 center">
                    <?= ($fund['income'] > 0)
                        ? $this->formatNumber($fund['income']) . _afghani
                        : $this->formatNumber($fund['income']) ?>
                </div>
            </div>
        </div>
        <div class="details mb30">
            <div class="detail-item d-flex align-center">
                <div class="w100 m10 center">موجودی صندوق اصلی</div>
                <div class="w100 m10 center">
                    <?= ($fund['total'] > 0)
                        ? $this->formatNumber($fund['total']) . _afghani
                        : $this->formatNumber($fund['total']) ?>
                </div>
            </div>
        </div>

        <div class="details mb30 bg-success">
            <div class="detail-item d-flex align-center">
                <div class="w100 m10 center"> مجموع موجودی <span class="fs12 color-orange">(دخل و صندوق)</span></div>
                <div class="w100 m10 center">
                    <?= ($total > 0)
                        ? $this->formatNumber($total) . _afghani
        ...
        <form action="<?= url('transfer-to-main-fund') ?>" method="post" class="mt20">
            <div class="center">
                <input type="hidden" name="csrf_token" value="<?= $_SESSION['csrf_token'] ?>">
                <input type="submit" class="btn p5" value="انتقال تمام موجودی دخل به صندوق">
            </div>
        </form>
            ...
            <div class="child-accordioin">
                <table class="fl-table">
                    ...
                    <tbody>
                        <?php
                        $perPage = 10;
                        $data = paginate($fundTransactions, $perPage);
                ...
                <div class="flex-justify-align mt20 paginate-section mb10">
                    <div class="table-info fs12">تعداد کل: <?= count($fundTransactions) ?></div>
                    <?php
                    if (count($fundTransactions) == null) { ?>
                        <div class="center color-red fs12">
                            <i class="fa fa-comment"></i>
                            <?= _not_infos ?>
                        </div>
                    <?php } else {
                        if (count($fundTransactions) > 10) {
                            echo paginateView($fundTransactions, 10);
                        }
                    }
                    ?>
                </div>
