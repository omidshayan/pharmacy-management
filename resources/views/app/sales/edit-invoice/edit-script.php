...
<script>
    $(document).ready(function() {
        ...
        $(document).on('click', `${resultContainerSelector} li`, function() {
            ...
            $.ajax({
                url: '<?= url("edit-invoice-item-store") ?>',
                method: 'POST',
                data: {
                    product_id: item.id,
                    ...
                    invoice_id: $('#select_invoice_id').val(),
                },
                dataType: 'json',
...
<script>
    ...
    function loadInvoiceItems(id) {
        const finalId = id || '<?= $sale_invoice['id'] ?>';

        $('#cart-items-tbody').html('<tr><td colspan="10" class="text-center py-4"><div class="spinner-border text-primary"></div></td></tr>');

        toggleSubmitButton([], true);

        $.ajax({
            url: '<?= url("get-edit-invoice-items-ajax") ?>/' + finalId,
            type: 'GET',
            dataType: 'json',
            success: function(response) {
                if (response.success) {
            ...
            error: function() {
                $tbody.html('<tr><td colspan="10" class="text-center color-red">خطا در برقراری ارتباط با سرور</td></tr>');
...
<script>
    ...
    $(document).on('input', '.package-qty, .unit-qty, .unit-price-input, .package-price-input', function() {
        ...
        ajaxTimeout = setTimeout(() => {
            ...
            $.ajax({
                url: '<?= url("update-edit-invoice-item") ?>/' + itemId,
                method: 'POST',
                data: {
...
<script>
    $(document).on('click', '.delete-product-cart', function() {
        ...
        $.ajax({
            url: '<?= url("edit-delete-product-cart") ?>/' + itemId,
            dataType: 'json',
            success: function(response) {
...
<!-- show btn or section -->
<script>
    function toggleSubmitButton(items, isLoading = false) {
        const $submitSection = $('.show-hide'); // بخش دکمه‌ها
        const $loadingSection = $('#submit-loading'); // بخش لودینگ جدید
        const $inputs = $('.input-disable'); // فیلدهای ورودی
        ...
        if (isLoading) {
            ...
            // ۲. پایان بارگذاری: لودینگ را مخفی کن
            $loadingSection.addClass('d-none');

            if (items && items.length > 0) {
                // اگر آیتم داشتیم، دکمه‌ها را نشان بده
                $submitSection.removeClass('d-none');
                $inputs.prop('disabled', false);
            } else {
                // اگر آیتم نداشتیم، دکمه‌ها مخفی بمانند
                $submitSection.addClass('d-none');
                $inputs.prop('disabled', true);
            }
        }
