...
<!-- Start content -->
<div class="content">
    <div class="content-title"> نمایش معاشات
    ...
    <div class="content-container">
        <table class="fl-table">
            ...
            <tbody>
                ...
                foreach ($data as $item) {
                    $monthName = isset($zodiacMonths[$item['month']]) ? $zodiacMonths[$item['month']] : 'نامشخص';
                ?>
                    <tr>
                        <td class="<?= ($item['status'] != 1) ? 'bg-red' : '' ?> color-orange"><?= $this->formatNumber($number) ?></td>
                        <td><?= $item['employee_name'] ?></td>
                        <td><?= $monthName ?></td>
                        <td><?= jdate('Y/m/d', $item['date']) ?></td>
                        <td>
                            <?=
                            ($item['transaction_type'] == 1) ? 'معاش' : (($item['transaction_type'] == 2) ? 'اضافه کاری' : (($item['transaction_type'] == 3) ? 'کسری' : 'نامشخص'))
                            ?>
                        </td>
                        <td><?= $this->formatNumber($item['amount']) ?></td>
                        <td>
                            <a href="<?= url('edit-salary/' . $item['id']) ?>" class="color-orange">
        ...
        <div class="flex-justify-align mt20 paginate-section">
            <div class="table-info fs14">تعداد کل: <?= $this->formatNumber(count($salaries)) ?></div>
            <?php
