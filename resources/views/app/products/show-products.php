    ...
    <div class="flex-justify-align">
        <div class="border search-database-s flex-justify-align">
            ...
            <input
                ...
                data-search-url="<?= url('search-product') ?>"
    ...
    <div class="content-container mt20">
        <table class="fl-table">
            ...
            <tbody>
                <?php
                $perPage = 10;
                $data = paginate($products, $perPage);
                $currentPage = isset($_GET['page']) ? (int)$_GET['page'] : 1;
                $number = ($currentPage - 1) * $perPage + 1;
                foreach ($data as $product) {
                ?>
                    <tr>
                        <td class="color-orange <?= statusColor($product['status']) ?>"><?= $number ?></td>
                        <td><?= $product['product_name'] ?></td>
                        <td><?= ($product['product_cat'] !== '') ?  $product['product_cat'] : '- - - -' ?></td>
                        <td><?= ($product['package_type'] !== '') ?  $product['package_type'] : '- - - -' ?></td>

                        <td>
                            <a href="<?= url('edit-product/' . $product['id']) ?>" class="color-orange">
        ...
        <div class="flex-justify-align mt20 paginate-section">
            <div class="table-info fs14">تعداد کل: <?= count($products) ?></div>
            <?php
